...
class WMBusFrame:
    ...
    def log(self, verb):
        ...
        0: just single line
        1: additionally log frame header and transport header info
        2: additionally log data records
        """
        line = datetime.now().strftime("%b %d %H:%M:%S") + " "
        line += str(self.get_manufacturer_short()) + " "
        ...
        if self.records:
            ...
            if verb >= 1:
                line += '\n--'
                line += "\nCI Detail:\t" + util.tohex(
                    self.control_information) + " (" + self.get_ci_detail() + ", " + self.get_function_code() + ")"
    ...
    def get_vif_description(self):
        ...
        if (extension):
            return {
                ...
                # True VIF is given in the first VIFE and is coded using 128 new VIF-Codes (table 29)
                0xFD: 'Second extension of VIF-codes',
                # True VIF is given in the first VIFE and is coded using 128 new VIF-Codes (tabke 28)
                0xEF: 'Reserved extension'
                # Reserved for third extension table of VIF-codes for a future table especially for electricity meters
        ...
        return {
            ...
            # Allows user definable VIF ́s (in plain ASCII-String)
...
class WMBusDataRecord:
    ...
    def get_power_in_w(self):
        chooser = self.header.vif[0] & 0x7F
        value = int.from_bytes(self.value, "little")
        return value * {
            0x28: 1000,  # Power mW
            0x29: 100,  # Power 10⁻² W
            0x2A: 10,  # Power 10⁻¹ W
            0x2B: 1,  # Power W
            0x2C: 0.1,  # Power 10¹ W
            0x2D: 0.01,  # Power 10² W
            0x2E: 0.001,  # Power kW
            0x2F: 0.0001,  # Power 10⁴ W',
        }.get(chooser)

    def get_energy_in_wh(self):
        chooser = self.header.vif[0] & 0x7F
        value = int.from_bytes(self.value, "little")

        return value * {
            0x00: 1000,  # Energy mWh
            0x01: 100,  # Energy 10⁻² Wh
            0x02: 10,  # Energy 10⁻¹ Wh
            0x03: 1,  # Energy Wh
            0x04: 0.1,  # Energy 10¹ Wh
            0x05: 0.01,  # Energy 10² Wh
            0x06: 0.001,  # Energy kWh
            0x07: 0.00001,  # Energy 10⁴ Wh
        }.get(chooser)
